...
class StandardizerAgent:
    ...
    def __init__(self, openai_service: OpenAIService, enable_standardization: bool = True, debug_mode: bool = False):
    ...
    def _build_prompt(self, summary: str) -> str:
        """
        构造标准化 prompt。
        优先调用 openai_service.get_prompt("structured_output", {"text": summary}) 加载 YAML 模板，
        并将预设 JSON 模板反向注入到提示中，确保 GPT 输出符合 JSON 格式要求。
    ...
    def _slice_text(self, text: str, max_length: int = DEFAULT_MAX_LENGTH) -> List[str]:
        """
        对长文本进行切片，确保每个片段长度不超过 max_length。

        :param text: 输入文本
        :param max_length: 每个片段的最大字符数
        :return: 文本片段列表
        """
    ...
    def _merge_results(self, results: List[Any]) -> Any:
        """
        合并多个片段的标准化结果。当前示例简单返回第一个有效结果，
        后续可根据业务需要设计更复杂的合并策略。

        :param results: 各片段处理结果列表
        :return: 合并后的结果
        """
        for res in results:
            if res is not None:
    ...
    async def standardize(self, summary: str, model: str = "gpt-4") -> Any:
        """
        异步调用 OpenAI API，将输入摘要转换为结构化输出。
        若文本较长则进行切片处理，逐片调用标准化流程后合并结果。

        :param summary: 待标准化的摘要文本
        :param model: 使用的 OpenAI 模型名称
        :return: 标准化输出（若能解析为 JSON，则返回字典；否则返回原始文本）
        """
...
def standardizer_agent(summary: str, model: str = "gpt-4", openai_service: Optional[OpenAIService] = None, debug_mode: bool = False) -> asyncio.Future:
    """
    工厂函数：异步调用 StandardizerAgent.standardize 方法。

    :param summary: 待标准化的摘要文本
    :param model: 使用的 OpenAI 模型名称
    :param openai_service: 已初始化且加载 YAML 配置的 OpenAIService 实例（必须提供）
    ...
    agent = StandardizerAgent(openai_service=openai_service, debug_mode=debug_mode)
